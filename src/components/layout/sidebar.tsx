...
export function Sidebar() {
  ...
  // Memoize expensive selector calculations
  const blockedTasks = useMemo(() => 
    isLoaded ? getBlockedTasks(state) : [], 
    [isLoaded, state.completedIds, state.taskMeta]
  );
  ...
  const overallStats = useMemo(() => 
    isLoaded ? getOverallStats(state) : { percentage: 0 }, 
    [isLoaded, state.completedIds]
  );
  ...
  const missingRequiredDocs = useMemo(() => 
    isLoaded 
      ...
      : 0,
    [isLoaded, state.completedDocIds]
  );
