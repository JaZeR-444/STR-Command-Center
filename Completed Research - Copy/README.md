...
## The Archive System
The repository has two archive mechanisms:

1. **ARCHIVE/** - Root-level archive folder containing historical backups and deprecated system files
2. **`.ARCHIVE`** - Subfolders within ACTIVE/ sections that store older, preliminary, or temporary documentation

All archived files exist only for reference and must never be edited or promoted to active use.
...
- the appropriate ACTIVE/ section folder for any current related document
- the corresponding `.ARCHIVE` subfolder for historical reference only
- SUPPORT/Prompts/ for relevant prompt packs
- SUPPORT/Checklists/ for section completion tracking
...
### 3. Create or update only in ACTIVE/ sections
If the task is to produce a usable document, the output must exist in the appropriate ACTIVE/ section folder.

Examples:
- `ACTIVE/01_Planning/`
- `ACTIVE/02_Design/`
- `ACTIVE/08_Revenue/`
- `ACTIVE/09_Compliance/`

No final deliverable should live only inside ARCHIVE/ or `.ARCHIVE` subfolders.
...
- **Inbox/** - Incoming items and new research
- **Intake/** - Processing and triage
- **Prompts/** - AI prompts and automation workflows
- **Checklists/** - Task lists and completion tracking
- **Command_Center/** - Project dashboards and control panels
- **Validation/** - Quality checks and validation reports

### ARCHIVE/ Reference

- **Backups/** - Backup copies and snapshots
- **Historical_Versions/** - Old versions and deprecated content
- **System_Archive/** - Archived system governance files

### _SYSTEM/ Governance

- **Policies/** - Repository structure standards and rules
- **Schemas/** - Data format specifications
- **Registers/** - Master indexes and change logs
- **Templates/** - Section templates and starting point files
...
- **New to this repository?** → Start with `START_HERE.md`
- **Need to find something?** → Check `INDEX.md`
- **What's the current state?** → Review `STATUS.md`
- **Looking for a specific document?** → See `FILE_REGISTER.md`
