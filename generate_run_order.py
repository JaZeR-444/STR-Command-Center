...
output_file = os.path.join(base_dir, "MASTER_PROMPT_RUN_ORDER.md")

categories = sorted([d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d)) and d[0:2].isdigit()])

with open(output_file, 'w', encoding='utf-8') as f:
    f.write("# STR Master Prompt Run Order\n\n")
    f.write("This document provides the definitive, step-by-step execution order for the 11-module STR Launch Command Center prompt pack. Following this sequence ensures complete documentation generation without missing steps or circular dependencies.\n\n")
    f.write("---\n\n")
    ...
    for category in categories:
        ...
        # Find Execution prompts (items that don't match the standard structural prefixes)
...
print(f"Run order generated at: {output_file}")
